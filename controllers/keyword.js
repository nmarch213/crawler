                        ...
                        setTimeout(function() {
                            ...
                            for (var i = 0; i < googleSearchCiteResults.length; i++) {
                                if (googleSearchCiteResults[i].includes(rootUrl[1])) {
                                    ...
                                    Keyword.findByIdAndUpdate({_id:foundKeyword._id}, {$push: {rank: realRank, websiteFound: googleSearchCiteResults[i], date: moment().format('MMM Do YYYY, h:mm:ss a')}}, function(err, updatedKeyword){
